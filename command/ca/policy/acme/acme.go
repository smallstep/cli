...
func Command(ctx context.Context) cli.Command {
	return cli.Command{
		...
		Subcommands: cli.Commands{
			actions.ViewCommand(policycontext.NewContextWithACMEPolicyLevel(ctx)),
			actions.RemoveCommand(policycontext.NewContextWithACMEPolicyLevel(ctx)),
			x509.Command(policycontext.NewContextWithACMEPolicyLevel(ctx)),
		},
	}
}
